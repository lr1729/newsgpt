...
async function main() {
  ...
  if (sourceArticlesData.length > 0) {
    console.log(`\n--- Step 4: Generating Analysis for ${sourceName} (${sourceArticlesData.length} articles) ---`);
    const modelUsedSuffix = argv.model ? argv.model.replace(/[^a-zA-Z0-9]/g,'_') : 'default';

    const { combinedContent } = await aggregateArticleData(sourceArticlesData);
    ...
    if (combinedContent) {
      ...
      console.log(`🚫 Aggregated content was empty for ${sourceName}. No analysis generated.`);
    }
  } else {
    console.log(`\n🚫 No articles processed successfully for ${sourceName}. No analysis generated.`);
  }
